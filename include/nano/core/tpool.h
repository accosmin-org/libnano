...
namespace nano
{
    ...
    public:
        ...
        ///
        static size_t size() { return std::max(size_t(1), static_cast<size_t>(std::thread::hardware_concurrency())); }
    ...
    ///
    template <typename tsize, typename tchunksize, typename toperator,
    ...
    void loopr(tsize size, tchunksize _chunk, const toperator& op, bool raise = true)
    {
        ...
        {
            section.push_back(pool.enqueue(
                [&op, size = size, chunk = chunk, tchunk = tchunk, tbegin = tbegin](size_t tnum)
                {
    ...
    void loopi(tsize size, const toperator& op, bool raise = true)
    {
        ...
        {
            section.push_back(pool.enqueue(
                [&op, size = size, tchunk = tchunk, tbegin = tbegin](size_t tnum)
                {
