...
auto make_marker_maximum_percentage_cols(const tscalar percentage)
{
    return [percentage = percentage](const row_t& row)
    {
        ...
        return (it == values.end())
                 ? std::vector<size_t>{}
                 : detail::filter_greater(
                       values, it->second - percentage * (it->second < 0 ? -it->second : +it->second) / tscalar(100));
    };
}
...
auto make_marker_minimum_percentage_cols(const tscalar percentage)
{
    return [percentage = percentage](const row_t& row)
    {
        ...
        return (it == values.end())
                 ? std::vector<size_t>{}
                 : detail::filter_less(values, it->second + percentage * (it->second < 0 ? -it->second : +it->second) /
                                                                tscalar(100));
    };
}
} // namespace nano
