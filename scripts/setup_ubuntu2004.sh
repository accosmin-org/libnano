...
function setup {
    ...
    sudo apt update -qq
    sudo apt install -y git vim cmake lcov cppcheck valgrind ninja-build iwyu
...
options:
    ...
    --gcc [version]
        install additional supported gcc versions (5.0, 6.0, 7, 8, 9, 10, 11, 12, 13)
    --llvm [version]
        install additional supported llvm versions (5.0, 6.0, 7, 8, 9, 10, 11, 12, 13)
EOF
    exit 1
}
