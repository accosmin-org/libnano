...
UTEST_CASE(evaluate)
{
    ...
    const auto inputs = iterator.inputs(make_fold(), 0, iterator.samples(make_fold()));
    const auto targets = iterator.targets(make_fold(), 0, iterator.samples(make_fold()));
...
UTEST_CASE(minimize_cgd)
{
    ...
    solver->logger([] (const solver_state_t& state)
    {
        std::cout << "i=" << state.m_iterations << ", calls=" << state.m_fcalls << "/" << state.m_gcalls
            ...
            << ", status=" << state.m_status << ", lrate=" << state.lrate << ", decay=" << state.decay << ".\n";
        return true;
    });
...
UTEST_CASE(minimize_sgd)
{
    auto loss = make_loss();
    auto solver = make_solver("sgd", 1e-3);
    auto dataset = make_dataset(3, 2);
    auto iterator = memfixed_iterator_t<scalar_t>{dataset};
...
UTEST_CASE(train)
{
    ...
    for (const auto normalization : enum_values<::nano::normalization>())
    {
        for (const auto regularization : enum_values<::nano::regularization>())
        {
            ...
            UTEST_REQUIRE_NOTHROW(training = model.train(*loss, iterator, *solver));
            ...
            for (const auto& train_fold : training)
            {
                ...
                UTEST_CHECK_LESS_EQUAL(train_fold.m_tr_error, 1e+2 * solver->epsilon());
                UTEST_CHECK_LESS_EQUAL(train_fold.m_vd_error, 1e+2 * solver->epsilon());
                UTEST_CHECK_LESS_EQUAL(train_fold.m_te_error, 1e+2 * solver->epsilon());
            }
    ...
    UTEST_CHECK_THROW(model.train(*loss, iterator, *solver), std::runtime_error);
}
