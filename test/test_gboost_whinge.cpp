...
public:
    ...
    void make_target(const tensor_size_t sample) override
    {
        target(sample).constant(
            make_hinge_target(sample, feature(), 5, 2.5, +3.0, -2.1, 0));
    }
    ...
    {
        return {make_dims(4, 1, 1, 1), {+3.0, -3.0 * threshold(), -2.1, +2.1 * threshold()}};
    }
};
...
UTEST_CASE(fitting)
{
    const auto dataset = make_dataset<whinge_dataset_t>();
    const auto datasetx1 = make_dataset<whinge_dataset_t>(dataset.isize(), dataset.tsize() + 1);
    const auto datasetx2 = make_dataset<whinge_dataset_t>(dataset.feature(), dataset.tsize());
    const auto datasetx3 = make_dataset<no_continuous_features_dataset_t<whinge_dataset_t>>();
