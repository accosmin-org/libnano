...
[[maybe_unused]] static void check_select0(const select_iterator_t& iterator,
    indices_cmap_t samples, indices_cmap_t features, const tensor_t<tstorage, tscalar, trank>& expected)
{
    ...
    {
        UTEST_CHECK_NOTHROW(iterator.loop(samples, features, [&] (tensor_size_t feature, size_t, tvalues values)
        {
...
[[maybe_unused]] static void check_select(const select_iterator_t& iterator,
    indices_cmap_t samples, indices_cmap_t features, const sclass_mem_t& expected)
{
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, mclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, scalar_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, struct_cmap_t) {}), std::runtime_error);
    check_select0(iterator, samples, features, expected);
}

[[maybe_unused]] static void check_select(const select_iterator_t& iterator,
    indices_cmap_t samples, indices_cmap_t features, const mclass_mem_t& expected)
{
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, sclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, scalar_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, struct_cmap_t) {}), std::runtime_error);
    check_select0(iterator, samples, features, expected);
}
...
[[maybe_unused]] static void check_select(const select_iterator_t& iterator,
    indices_cmap_t samples, indices_cmap_t features, const scalar_mem_t& expected)
{
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, sclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, mclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, struct_cmap_t) {}), std::runtime_error);
...
[[maybe_unused]] static void check_select(const select_iterator_t& iterator,
    indices_cmap_t samples, indices_cmap_t features, const struct_mem_t& expected)
{
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, sclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, mclass_cmap_t) {}), std::runtime_error);
    UTEST_CHECK_THROW(iterator.loop(samples, features, [&] (tensor_size_t, size_t, scalar_cmap_t) {}), std::runtime_error);
...
template <template <typename, size_t> class tstorage, typename tscalar, size_t trank>
[[maybe_unused]] static void check_select(const dataset_generator_t& generator,
    tensor_size_t feature, const tensor_t<tstorage, tscalar, trank>& expected)
{
        ...
        for (const auto& samples : make_samples(generator))
        {
            for (const auto batch : {1, 3, 8})
            {
                ...
                for (const auto scaling : enum_values<scaling_type>())
                {
                    ...
                    UTEST_CHECK_NOTHROW(iterator.loop([&] (tensor_range_t range, size_t tnum, tensor2d_cmap_t flatten)
                    {
    ...
    const indices_t& expected_struct_features)
{
    ...
    const auto make_features = [&] () { return map_tensor(features.data(), static_cast<tensor_size_t>(features.size())); };
    ...
    for (const auto execution : {execution_type::par, execution_type::seq})
    {
        for (const auto batch : {1, 3, 8})
        {
            ...
            for (const auto scaling : enum_values<scaling_type>())
            {
                ...
                auto called = make_full_tensor<tensor_size_t>(make_dims(samples.size()), 0);
                UTEST_CHECK_NOTHROW(iterator.loop([&] (tensor_range_t range, size_t tnum, tensor4d_cmap_t targets)
                {
                    called.slice(range).full(1);
                    UTEST_CHECK_GREATER_EQUAL(tnum, 0U);
                    UTEST_CHECK_LESS(tnum, tpool_t::size());
