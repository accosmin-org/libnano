...
UTEST_CASE(lasso_fpba1_case1)
{
    // NB: test case generated by solving a lasso problem using FPBA1 solver!
    const auto xvec = make_vector<double>(0.0003235385, 0.0011213438, 0.0003235385, 0.0003235385, 0.0003235385,
                                          0.0000000000, 0.0008329018, 0.0002319789, 0.0000892558, 0.0002316027);

    auto       deleted   = tensor_size_t{0};
    const auto to_delete = tensor_size_t{2};
    const auto threshold = 0.0008329018;

    const auto op = [&](const tensor_size_t i)
    {
        if (deleted < to_delete)
        {
            const auto ret = xvec(i) >= threshold;
            deleted += ret ? 1 : 0;
            return ret;
        }
        else
