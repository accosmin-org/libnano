...
{
    check_minimize(make_solvers(), function_t::make({4, 4, convexity::yes, smoothness::yes}));
}

UTEST_CASE(nonsmooth_bundle)
{
    check_minimize(make_solvers(), function_t::make({4, 4, convexity::yes, smoothness::no}));
}
