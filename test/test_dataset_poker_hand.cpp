...
UTEST_BEGIN_MODULE(test_dataset_poker_hand)

UTEST_CASE(load)
{
    auto dataset = tabular_dataset_t::all().get("poker-hand");
    ...
    UTEST_CHECK_EQUAL(dataset->ifeatures(), 10);
    ...
    for (size_t f = 0, folds = dataset->folds(); f < folds; ++ f)
    {
        ...
        const auto tr_size = 60 * 25010 / 100;
        const auto vd_size = 25010 - tr_size;
        const auto te_size = 1000000;

        UTEST_CHECK_EQUAL(tr_inputs.dims(), make_dims(tr_size, 10, 1, 1));
        UTEST_CHECK_EQUAL(vd_inputs.dims(), make_dims(vd_size, 10, 1, 1));
        UTEST_CHECK_EQUAL(te_inputs.dims(), make_dims(te_size, 10, 1, 1));

        UTEST_CHECK_EQUAL(tr_targets.dims(), make_dims(tr_size, 10, 1, 1));
        UTEST_CHECK_EQUAL(vd_targets.dims(), make_dims(vd_size, 10, 1, 1));
        UTEST_CHECK_EQUAL(te_targets.dims(), make_dims(te_size, 10, 1, 1));
    }
}
