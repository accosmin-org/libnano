...
UTEST_CASE(select_scalar)
{
    const auto dataset = make_dataset(10, string_t::npos);
    {
        ...
        const auto expected_mapping = make_tensor<tensor_size_t>(make_dims(3, 5),
            5, 0, 1, 1, 1,
            6, 0, 1, 1, 1,
            7, 0, 1, 1, 1);
    ...
    }
    {
        ...
        const auto expected_mapping = make_tensor<tensor_size_t>(make_dims(1, 5),
            6, 0, 1, 1, 1);
        UTEST_CHECK_EQUAL(mapping, expected_mapping);
    }
}
...
UTEST_CASE(select_mclass)
{
    const auto dataset = make_dataset(10, string_t::npos);
    {
        ...
        const auto expected_mapping = make_tensor<tensor_size_t>(make_dims(2, 5),
            0, 3, 1, 1, 1,
            1, 4, 1, 1, 1);
        UTEST_CHECK_EQUAL(mapping, expected_mapping);
    }
