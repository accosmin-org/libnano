...
UTEST_CASE(config)
{
    auto dataset = dataset_t::all().get("wine");
...
UTEST_CASE(load)
{
    ...
    for (size_t f = 0, folds = dataset->folds(); f < folds; ++ f)
    {
        ...
        UTEST_CHECK_EQUAL(vd_inputs.dims(), make_dims(53, 13, 1, 1));
        UTEST_CHECK_EQUAL(te_inputs.dims(), make_dims(19, 13, 1, 1));

        UTEST_CHECK_EQUAL(tr_targets.dims(), make_dims(106, 3, 1, 1));
        UTEST_CHECK_EQUAL(vd_targets.dims(), make_dims(53, 3, 1, 1));
        UTEST_CHECK_EQUAL(te_targets.dims(), make_dims(19, 3, 1, 1));
    }
}
