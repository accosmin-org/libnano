...
feature_t base_elemwise_generator_t::make_scalar_feature(tensor_size_t ifeature, const char* name) const
{
    ...
    return  feature_t{scat(name, "(", feature.name(), ")")}.
...
feature_t base_elemwise_generator_t::make_mclass_feature(tensor_size_t ifeature, const char* name, strings_t labels) const
{
    assert(ifeature >= 0 && ifeature < features());
    const auto original = mapped_original(ifeature);
...
feature_t base_elemwise_generator_t::make_struct_feature(tensor_size_t ifeature, const char* name, tensor3d_dims_t dims) const
{
    assert(ifeature >= 0 && ifeature < features());
    const auto original = mapped_original(ifeature);
