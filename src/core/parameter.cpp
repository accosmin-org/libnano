...
{
    critical(
        std::find(param.m_domain.begin(), param.m_domain.end(), value) == param.m_domain.end(),
        "parameter (", name, "): out of domain enumeration value, !('",
        value, "' in [", scat(param.m_domain), "])");
...
static auto& update(const string_t& name, parameter_t::range_t<tscalar>& param, tvalue value_)
{
    ...
    critical(
        !std::isfinite(value) ||
        !::check(param.m_mincomp, param.m_min, value) ||
        !::check(param.m_maxcomp, value, param.m_max),
        "parameter (", name, "): out of domain scalar value, !(",
        param.m_min, ::name(param.m_mincomp), value_, ::name(param.m_maxcomp), param.m_max, ")");
...
static auto& update(const string_t& name, parameter_t::pair_range_t<tscalar>& param, tvalue1 value1_, tvalue2 value2_)
{
    ...
    critical(
        !std::isfinite(value1) ||
        !std::isfinite(value2) ||
        !::check(param.m_mincomp, param.m_min, value1) ||
        !::check(param.m_valcomp, value1, value2) ||
        !::check(param.m_maxcomp, value2, param.m_max),
        "parameter (", name, "): out of domain pair of scalar values, !(",
        param.m_min, ::name(param.m_mincomp), value1_, ::name(param.m_valcomp),
        value2_, ::name(param.m_maxcomp), param.m_max, ")");
...
static void update(const string_t& name, parameter_t::storage_t& storage, tvalue value)
{
    std::visit(overloaded{
        [&] (parameter_t::irange_t& param) { ::update(name, param, value); },
...
static void update(const string_t& name, parameter_t::storage_t& storage, std::tuple<tvalue, tvalue> value)
{
    ...
    std::visit(overloaded{
        [&] (parameter_t::iprange_t& param) { ::update(name, param, value1, value2); },
        [&] (parameter_t::fprange_t& param) { ::update(name, param, value1, value2); },
        [&] (auto&) { critical0("parameter (", name, "): cannot set value (", value1, ",", value2, ")!"); }
    }, storage);
}
...
static auto read(const string_t& name, std::istream& stream, parameter_t::pair_range_t<tscalar>)
{
    ...
    critical(
        !::nano::read(stream, value1) ||    // LCOV_EXCL_LINE
    ...
    return parameter_t::pair_range_t<tscalar>{
        value1, value2, min, max, make_comp(minLE), make_comp(valueLE), make_comp(maxLE)};
}

template <typename tscalar>
static void write(
    const string_t& name, std::ostream& stream, int32_t type, const parameter_t::range_t<tscalar>& param)
{
    critical(
        !::nano::write(stream, type) ||
        !::nano::write(stream, name) ||
...
template <typename tscalar>
static void write(
    const string_t& name, std::ostream& stream, int32_t type, const parameter_t::pair_range_t<tscalar>& param)
{
    critical(
        !::nano::write(stream, type) ||
        !::nano::write(stream, name) ||
...
static bool operator==(const parameter_t::range_t<tscalar>& lhs, const parameter_t::range_t<tscalar>& rhs)
{
    return  lhs.m_value == rhs.m_value &&
            lhs.m_min == rhs.m_min && make_flag(lhs.m_mincomp) == make_flag(rhs.m_mincomp) &&
            lhs.m_max == rhs.m_max && make_flag(lhs.m_maxcomp) == make_flag(rhs.m_maxcomp);
}
...
static bool operator==(const parameter_t::pair_range_t<tscalar>& lhs, const parameter_t::pair_range_t<tscalar>& rhs)
{
    return  lhs.m_value1 == rhs.m_value1 &&
            lhs.m_value2 == rhs.m_value2 && make_flag(lhs.m_valcomp) == make_flag(rhs.m_valcomp) &&
            lhs.m_min == rhs.m_min && make_flag(lhs.m_mincomp) == make_flag(rhs.m_mincomp) &&
            lhs.m_max == rhs.m_max && make_flag(lhs.m_maxcomp) == make_flag(rhs.m_maxcomp);
}
...
{
    return stream
        << param.m_min << " " << ::name(param.m_mincomp) << " "
        << param.m_value << " " << ::name(param.m_maxcomp) << " " << param.m_max;
}
...
static std::ostream& domain(std::ostream& stream, const parameter_t::pair_range_t<tscalar>& param)
{
    return stream
        << param.m_min << " " << ::name(param.m_mincomp) << " "
        << param.m_value1 << " " << ::name(param.m_valcomp) << " "
        << param.m_value2 << " " << ::name(param.m_maxcomp) << " " << param.m_max;
}
...
{
    std::visit(overloaded{
        [&] (enum_t& param) { ::update(m_name, param, std::move(value)); },
...
{
    using ::operator==;
    return lhs.name() == rhs.name() && std::visit(overloaded{
        [&] (const std::monostate&) { return std::get_if<std::monostate>(&rhs.storage()) != nullptr; },
...
{
    std::visit(overloaded{
        [&] (const std::monostate&) { stream << "N/A"; },
