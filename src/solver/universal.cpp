...
solver_pgm_t::solver_pgm_t()
{
    ...
    register_parameter(parameter_t::make_float("solver::pgm::D", 0.0, LT, 1.0, LT, max));
    register_parameter(parameter_t::make_float("solver::pgm::L0", 0.0, LT, 1.0, LT, max));
    register_parameter(parameter_t::make_integer("solver::pgm::lsearch_max_iterations", 10, LE, 50, LE, 100));
}

solver_state_t solver_pgm_t::minimize(const function_t& function_, const vector_t& x0) const
{
    const auto D = parameter("solver::pgm::D").value<scalar_t>();
