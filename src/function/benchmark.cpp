...
function_factory_t& benchmark_function_t::all()
{
    ...
    std::call_once(flag, [] ()
    {
        ...
        manager.add<function_quadratic_t>("quadratic", "random quadratic function: f(x) = x.dot(a) + x * A * x, where A is PD");
        manager.add<function_rosenbrock_t>("rosenbrock", "Rosenbrock function: https://en.wikipedia.org/wiki/Test_functions_for_optimization");
        manager.add<function_exponential_t>("exponential", "exponential function: f(x) = exp(1 + x.dot(x) / D)");
        manager.add<function_dixon_price_t>("dixon-price", "Dixon-Price function: https://www.sfu.ca/~ssurjano/dixonpr.html");
        manager.add<function_chung_reynolds_t>("chung-reynolds", "Chung-Reynolds function: f(x) = (x.dot(x))^2");
        manager.add<function_axis_ellipsoid_t>("axis-ellipsoid", "axis-parallel hyper-ellipsoid function: f(x) = sum(i*x+i^2, i=1,D)");
        manager.add<function_styblinski_tang_t>("styblinski-tang", "Styblinski-Tang function: https://www.sfu.ca/~ssurjano/stybtang.html");
        manager.add<function_schumer_steiglitz_t>("schumer-steiglitz", "Schumer-Steiglitz No. 02 function: f(x) = sum(x_i^4, i=1,D)");
        manager.add<function_rotated_ellipsoid_t>("rotated-ellipsoid", "rotated hyper-ellipsoid function: https://www.sfu.ca/~ssurjano/rothyp.html");
        manager.add<function_geometric_optimization_t>("geometric", "generic geometric optimization function: f(x) = sum(i, exp(alpha_i + a_i.dot(x)))");
        manager.add<function_enet_mse_t>("mse+ridge", "mean squared error with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_mse_t>("mse+lasso", "mean squared error with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_mse_t>("mse+elasticnet", "mean squared error with elastic net-like regularization", 10, 1.0, 1.0);
        manager.add<function_enet_mae_t>("mae+ridge", "mean squared error with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_mae_t>("mae+lasso", "mean squared error with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_mae_t>("mae+elasticnet", "mean squared error with elastic net-like regularization", 10, 1.0, 1.0);
        manager.add<function_enet_hinge_t>("hinge+ridge", "hinge loss (linear SVM) with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_hinge_t>("hinge+lasso", "hinge loss (linear SVM) with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_hinge_t>("hinge+elasticnet", "hinge loss (linear SVM) with elastic net-like regularization", 10, 1.0, 1.0);
        manager.add<function_enet_cauchy_t>("cauchy+ridge", "cauchy loss (robust regression) with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_cauchy_t>("cauchy+lasso", "cauchy loss (robust regression) with lasso-like regularization", 10, 1.0, 0.0);
        ...
        manager.add<function_enet_logistic_t>("logistic+ridge", "logistic regression with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_logistic_t>("logistic+lasso", "logistic regression with lasso-like regularization", 10, 1.0, 0.0);
