...
function_factory_t& benchmark_function_t::all()
{
    ...
    std::call_once(flag, [] ()
    {
        manager.add<function_trid_t>(
            "trid", "Trid function: https://www.sfu.ca/~ssurjano/trid.html");
        manager.add<function_qing_t>(
            "qing", "Qing function: http://benchmarkfcns.xyz/benchmarkfcns/qingfcn.html");
        manager.add<function_kinks_t>(
            "kinks", "random kinks: f(x_ = sum(|x - K_i|, i)");
        manager.add<function_cauchy_t>(
        ...
        manager.add<function_schumer_steiglitz_t>(
        ...
        manager.add<function_enet_mse_t>(
            "mse+ridge", "mean squared error with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_mse_t>(
            "mse+lasso", "mean squared error with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_mse_t>(
            "mse+elasticnet", "mean squared error with elastic net-like regularization", 10, 1.0, 1.0);

        manager.add<function_enet_mae_t>(
            "mae+ridge", "mean squared error with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_mae_t>(
            "mae+lasso", "mean squared error with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_mae_t>(
            "mae+elasticnet", "mean squared error with elastic net-like regularization", 10, 1.0, 1.0);

        manager.add<function_enet_hinge_t>(
            "hinge+ridge", "hinge loss (linear SVM) with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_hinge_t>(
            "hinge+lasso", "hinge loss (linear SVM) with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_hinge_t>(
        ...
        manager.add<function_enet_logistic_t>(
            "logistic+ridge", "logistic regression with ridge-like regularization", 10, 0.0, 1.0);
        manager.add<function_enet_logistic_t>(
            "logistic+lasso", "logistic regression with lasso-like regularization", 10, 1.0, 0.0);
        manager.add<function_enet_logistic_t>(
