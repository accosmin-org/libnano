...
scalar_t function_chained_cb3I_t::do_eval(eval_t eval) const
{
    ...
    if (eval.has_grad())
    {
        ...
        for (tensor_size_t i = 0, dims = size(); i + 1 < dims; ++i)
        {
            ...
            if (v1 > std::max(v2, v3))
            {
                eval.m_gx(i) += 4.0 * cube(eval.m_x(i));
                eval.m_gx(i + 1) += 2.0 * eval.m_x(i + 1);
            }
            else if (v2 > std::max(v1, v3))
            {
                eval.m_gx(i) -= 4.0 - 2.0 * eval.m_x(i);
                eval.m_gx(i + 1) -= 4.0 - 2.0 * eval.m_x(i + 1);
            }
            else
            {
                const auto e = std::exp(eval.m_x(i + 1) - eval.m_x(i));
                eval.m_gx(i) -= 2.0 * e;
                eval.m_gx(i + 1) += 2.0 * e;
            }
        }
