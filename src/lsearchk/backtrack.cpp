...
{
    register_parameter(parameter_t::make_enum("lsearchk::backtrack::interpolation", interpolation::cubic));
}
...
bool lsearchk_backtrack_t::get(const solver_state_t& state0, solver_state_t& state)
{
    ...
    for (int i = 0; i < max_iterations && state; ++ i)
    {
        ...
        // next trial
        state.update(state0, lsearch_step_t::interpolate(state0, state, interp));
        log(state0, state);
    }
