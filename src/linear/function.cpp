...
static auto isize(const flatten_iterator_t& iterator) { return iterator.generator().columns(); }
static auto tsize(const flatten_iterator_t& iterator) { return ::nano::size(iterator.generator().target_dims()); }

linear::function_t::function_t(
    flatten_iterator_t iterator, const loss_t& loss, scalar_t l1reg, scalar_t l2reg, scalar_t vAreg) :
...
scalar_t linear::function_t::vgrad(const vector_t& x, vector_t* gx, vgrad_config_t) const
{
    ...
    m_iterator.loop([&] (tensor_range_t range, size_t tnum, tensor2d_cmap_t inputs, tensor4d_cmap_t targets)
    {
        ...
        if (gx != nullptr)
        {
            ...
            if (m_vAreg > 0.0)
            {
                ...
                cache.m_gW2.matrix() += gmatrix.transpose() * (imatrix.array().colwise() * vvector.array()).matrix();
            }
        }
    });
    ...
    return  cache0.m_vm1 +
            ((m_l1reg > 0.0) ? (m_l1reg * W.array().abs().mean()) : 0.0) +
