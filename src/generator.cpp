...
feature_t dataset_generator_t::target() const
{
    ...
    default: return m_dataset.visit_target([](const feature_t& feature, const auto&, const auto&) { return feature; });
    }
}
...
{
    critical(feature < 0 || feature >= features(), "dataset_generator_t: invalid feature index, expecting in [0, ",
             features(), "), got ", feature, "!");
}

void dataset_generator_t::check(indices_cmap_t samples) const
{
    critical(samples.min() < 0 || samples.max() > m_dataset.samples(),
