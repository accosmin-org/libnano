    ...
    wlearner_feature1_t::loopd(
        ...
        [&](tensor_size_t feature, const tensor1d_t& fvalues, tensor_size_t n_fvalues, size_t tnum)
        {
            ...
            for (tensor_size_t i = 0; i < fvalues.size(); ++i)
            {
                ...
                critical(fv < 0 || fv >= n_fvalues, "table weak learner: invalid feature value ", fv,
                         ", expecting [0, ", n_fvalues, ")");
...
void wlearner_table_t::predict(const dataset_t& dataset, const indices_cmap_t& samples, tensor4d_map_t outputs) const
{
    wlearner_feature1_t::predict(dataset, samples, outputs,
                                 [&](scalar_t x, tensor3d_map_t&& outputs)
                                 {
                                     ...
                                     critical(index < 0 || index >= fvalues(),
                                              "table weak learner: invalid feature value ", x, ", expecting [0, ",
                                              fvalues(), ")");
...
{
    return wlearner_feature1_t::split(dataset, samples, fvalues(),
                                      [&](scalar_t x)
                                      {
                                          const auto index = static_cast<tensor_size_t>(x);
                                          critical(index < 0 || index >= fvalues(),
