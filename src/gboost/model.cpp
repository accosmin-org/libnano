...
feature_infos_t gboost_model_t::features(const loss_t& loss, const dataset_t& dataset, const indices_t& samples,
                                         const solver_t& solver, importance type, tensor_size_t trials) const
{
    auto infos = features();
    ...
    log_info() << std::fixed << std::setprecision(6) << "gboost model: baseline error=" << baseline_error << ".";
    ...
    for (auto& info : infos)
    {
        ...
        default: critical0("gboost model: unhandled feature importance method!");
        }

        info.importance(feature_error - baseline_error);
        log_info() << std::fixed << std::setprecision(6) << "gboost model: feature[" << info.feature()
                   << "] error=" << feature_error << " vs. " << baseline_error << " => importance=" << info.importance()
                   << ".";
    }
