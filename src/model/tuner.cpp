...
tuner_t::step_t::step_t(tensor1d_t param, solver_state_t surrogate_fit, solver_state_t surrogate_opt,
...
{
    critical(!std::isfinite(m_value), "tuner: not finite value (", m_value, ") computed for parameters (",
             m_param.vector().transpose(), ")!");
...
{
    critical(m_param_spaces.empty(), "tuner: at least a parameter space is needed!");
...
tuner_t::steps_t tuner_t::optimize(const tensor2d_t& initial_params) const
{
    ...
    critical(initial_params.size<1>() != static_cast<tensor_size_t>(psize), "tuner: received ",
             initial_params.size<1>(), " parameters, expecting ", psize, "!");
    ...
    {
        steps.emplace_back(initial_params.tensor(sample), solver_state_t{}, solver_state_t{}, m_callback);
    ...
    for (tensor_size_t iteration = 0; iteration < max_iterations && steps.size() >= min_history; ++iteration)
    {
        ...
        auto       state_opt     = solver->minimize(
                      surrogate_opt, map(m_param_spaces, steps.rbegin()->m_opt_param.vector(), to_surrogate).vector());

        auto       param = map(m_param_spaces, state_opt.x, from_surrogate);
        const auto itdup =
            find_if(steps.begin(), steps.end(),
                    [&](const auto& step)
                    {
        ...
        steps.emplace_back(std::move(param), std::move(state_fit), std::move(state_opt), m_callback);
