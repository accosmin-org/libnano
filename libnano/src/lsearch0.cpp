...
lsearch0_factory_t& lsearch0_t::all()
{
    ...
    std::call_once(flag, [] ()
    {
        manager.add<lsearch0_linear_t>("linear", "linearly interpolate the previous line-search step");
        manager.add<lsearch0_constant_t>("constant", "constant line-search step length");
        manager.add<lsearch0_quadratic_t>("quadratic", "quadratically interpolate the previous line-search step");
